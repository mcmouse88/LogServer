def get_all_logs(connection):
    ...
    query = (
        "SELECT logs_table.log_id, logs_table.tag_name, tag_info_table.priority, logs_table.message, logs_table.timestamp, "
    ...
    response = []
    for (log_id, tag_name, priority, message, timestamp, color) in cursor:
        response.append(
            {
                'log_id': log_id,
                'tag_name': tag_name,
